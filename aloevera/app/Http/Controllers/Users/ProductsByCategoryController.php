...
class ProductsByCategoryController extends Controller
{
    ...
    public function index($id) {
        ...
        for($i = 0; $i < $count; $i++){
            ...
            //tính % giảm giá
             $products[$i]['sale'] = floor(((($products[$i]['price'])-($products[$i]['discount']))/($products[$i]['price'])) *100); 
             //Chuyển đổi tiền tệ
             $products[$i]['price'] = number_format($products[$i]['price']);
             $products[$i]['discount'] = number_format($products[$i]['discount']);
